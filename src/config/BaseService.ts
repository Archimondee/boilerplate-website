...
const refreshToken = async (): Promise<string | null> => {
  try {
    ...
    const response = await axios.post(
      `${import.meta.env.REACT_APP_API_URL}/api/${import.meta.env.REACT_APP_API_VERSION}/refresh-token`,
      {
        refreshToken,
      }
    );
...
const axiosInstance = axios.create({
  baseURL: `${import.meta.env.REACT_APP_API_URL}/api/${import.meta.env.REACT_APP_API_VERSION}`,
  timeout: 310000,
  headers: {
...
axiosInstance.interceptors.request.use(
  ...
  async (config: AxiosRequestConfig) => {
    ...
    let params = parseQueryParams(config.url || '');

    if (params) {
      console.info('Query Params:', params);
    }
    if (config.data) {
      console.info('Body:', config.data);
    }
